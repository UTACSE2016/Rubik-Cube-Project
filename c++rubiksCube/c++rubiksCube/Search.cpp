...
using namespace std;
 ...
 string Search::solution(string cubeString)
 {
	 int s;

	 FaceCube fc = FaceCube(cubeString);
	 ...
	 do {
		 ...
		 {
			 if (n == depthPhase1 - 1 && (s = totalDepth(depthPhase1, 20)) >= 0) 
			 {
				 return useSeparator ? solutionToString(s, depthPhase1) : solutionToString(s);  // s = depthPhase2 
			 }
